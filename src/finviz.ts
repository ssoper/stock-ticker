...
export const showSpyMap = async (
  page: puppeteer.Page,
  delaySecs: number = DefaultDelaySecs
): Promise<void> => {
...
const showPage = async (page: puppeteer.Page, url: string): Promise<void> => {
  ...
  await page.$eval(
    "div.content > div > div > div > div:nth-child(2)",
    (e, { marginTop }) => {
...
export const showFutures = async (
  page: puppeteer.Page,
  delaySecs: number = DefaultDelaySecs
): Promise<void> => {
